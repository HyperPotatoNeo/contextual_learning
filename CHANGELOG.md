...
- **`orchestrator.teacher_model.forward_kl`**: Added forward KL distillation mode. When enabled, the teacher generates its own rollouts (with teacher context) and the student is trained with an additional SFT loss to maximize likelihood of teacher-generated completions conditioned on the student prompt. `teacher_tau` controls the SFT weight. Mutually exclusive with reverse KL (default: False) (2026-03-15)
- **`orchestrator.teacher_model.sft_min_reward_gap`**: Added per-group SFT gating for forward KL. Only applies SFT loss for query groups where the teacher's average reward exceeds the student's by at least this threshold (default: None = no gating) (2026-03-15)
- **`transport.types.TrainingSample.sft_weight`**: Added per-sample SFT weight field for forward KL distillation (default: None) (2026-03-15)
- **`trainer.utils.DEFAULT_TIMEOUT`**: Increased default trainer timeout from 600s to 3600s (2026-03-15)
